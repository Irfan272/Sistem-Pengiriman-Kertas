...
class LaporanPengecekanController extends Controller
{
    public function index(){
        return view("laporan.pengecekan.index");
    }

    public function printPengecekan($tanggal_awal, $tanggal_akhir)
    {
        ...
        $pengecekan = Pengecekan_Mobil::with(['Supir'])
            ->whereBetween('tanggal_pengecekan', [$tanggal_mulai, $tanggal_terakhir])
            ->get();

        $total = $pengecekan->count();
        $tanggal_cetak = Carbon::today()->startOfDay()->format('d-m-Y');

        return view('laporan.pengecekan.cetak', compact('pengecekan', 'total', 'tanggal_mulai', 'tanggal_terakhir', 'tanggal_cetak'));
    }

    public function getPengecekanData(Request $request)
    {
        ...
        $data = Pengecekan_Mobil::whereBetween('tanggal_pengecekan', [$tanggal_awal, $tanggal_akhir])
            ->with(['Supir'])
            ->get();
